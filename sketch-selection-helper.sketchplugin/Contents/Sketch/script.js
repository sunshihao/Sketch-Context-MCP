...
function log(message) {
    if (typeof console !== "undefined" && console.log) {
        console.log("[Sketch Context MCP] " + message);
    }
}
...
function createPanel(options, context) {
    ...
    panel.setFrame_display_animate(
        NSMakeRect(0, 0, options.width, options.height),
        true,
        true,
    );
    ...
    panel
        ...
        .setHidden(!options.minimizable);
    panel
        .standardWindowButton(NSWindowZoomButton)
        .setHidden(!options.resizable);
...
WebViewMessageHandler.prototype.userContentController_didReceiveScriptMessage =
...
function copySelectionIDs() {
    try {
        ...
        showNotification(
            state.selection.length + " layer(s) copied to clipboard!",
        );
...
// Cleanup function to properly remove message handler
function cleanupPanel(panel) {
    try {
        if (panel && panel.webView) {
            var config = panel.webView.configuration();
            if (config) {
                var contentController = config.userContentController();
                if (contentController) {
                    // Remove the script message handler
                    contentController.removeScriptMessageHandlerForName(
                        "sketchPlugin",
                    );
                    log("Removed script message handler");
                }
            }
        ...
        log("Panel cleanup completed");
    } catch (error) {
        log("Error during panel cleanup: " + error);
    }
}

// Window close handler
function onCloseWindow(sender) {
    var panel = sender.window();
    log("Closing panel: " + panel);
    cleanupPanel(panel);
    panel.close();
}
